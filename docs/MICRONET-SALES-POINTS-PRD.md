...
### ☑ DONE PT1 — Server-authoritative `award_points` RPC
_Done 2026-06-15: migration **079_award_points_rpc** — SECURITY DEFINER `award_points(p_amount, p_reason, p_description)` caps each award at the matching active `points_rules` (`points_amount` + `cooldown_minutes` by `trigger_type = reason`), else clamps to 500; inserts `points_history` → the existing `points_history_sync_balance` trigger recomputes `user_points.balance`; granted to `authenticated`, revoked from anon/public. Client: `lib/points-sync.ts` `flushPointsOutbox()` now calls the RPC for each queued award when signed in (drains on success, exponential backoff on failure, stays queued when logged out) — the single chokepoint every earn already flows through (`enqueueSync`). The `PointsBadge` already reads server `user_points.balance`, so balances are now **server-authoritative + cross-device**; localStorage is the instant display cache. Build green. DISCOVERIES → PT2: client reasons (LISTENING/DAILY_BOUNTY/SHARE/VIDEO_WATCH/REFERRAL/KEYWORD_ENTRY/EVENT_CHECKIN/STREAK_BONUS) don't match the existing rule trigger_types (LISTEN_MINUTES/EVENT_ATTENDANCE/SIGNUP) yet, so PT1 uses the no-rule clamp path for current awards — PT2 aligns rules. Outbox carries idempotency keys but the RPC doesn't dedupe by key (rare double-award only on a lost-response retry); PT2 could add a `points_history` idempotency column if desired._
