...
const PHASES: Phase[] = [
  ...
  // =========================================================================
  {
    ...
    tasks: [
      ...
      { id: "p1-3", title: "Streams & channels schema", description: "streams table: name, slug, mount_point, genre, bitrate, status, listener_count, metadata JSONB, cover_image_url", status: "done", icon: Radio, tags: ["db"], effort: "M" },
      { id: "p1-4", title: "Shows & schedule schema", description: "shows table: title, description, host_id FK, genre, cover_art. schedule_slots table: show_id, day_of_week, start_time, end_time, stream_id, recurring boolean", status: "done", icon: CalendarDays, tags: ["db"], effort: "L" },
      { id: "p1-5", title: "Mixes & tracks schema", description: "mixes table: title, dj_id FK, file_url, duration, genre, bpm, waveform_url, play_count, status (draft/pending/published). tracks table for tracklists", status: "done", icon: Music, tags: ["db"], effort: "L" },
      { id: "p1-6", title: "Podcasts & episodes schema", description: "podcast_series: title, creator_id FK, cover_art, category, rss_url. episodes: series_id FK, title, audio_url, show_notes, duration, published_at", status: "done", icon: Mic, tags: ["db"], effort: "L" },
      { id: "p1-7", title: "Events & tickets schema", description: "events: title, description, date, location, type, capacity, price, cover_image. event_registrations: event_id, user_id, ticket_code, status, checked_in_at", status: "done", icon: Ticket, tags: ["db"], effort: "L" },
      { id: "p1-8", title: "Points & rewards schema", description: "point_transactions: user_id, amount, type (earn/spend), source, description. reward_items: name, point_cost, stock, image. reward_redemptions: user_id, item_id, status", status: "done", icon: Star, tags: ["db"], effort: "L" },
      { id: "p1-9", title: "Advertising schema", description: "ad_campaigns: advertiser_id, name, budget, start/end date, status. ad_creatives: campaign_id, type, file_url, status. ad_impressions: creative_id, stream_id, timestamp", status: "done", icon: Megaphone, tags: ["db"], effort: "XL" },
      { id: "p1-10", title: "Favorites, follows, & interactions", description: "favorites: user_id, item_type (show/mix/podcast/dj), item_id. follows: follower_id, following_id. listening_history: user_id, content_type, content_id, duration", status: "done", icon: Heart, tags: ["db"], effort: "M" },
      { id: "p1-11", title: "Community directory schema", description: "directory_listings: name, category, county, address, phone, website, claimed_by FK, claim_status. Already has frontend — needs backend table", status: "done", icon: MapPin, tags: ["db"], effort: "M" },
      { id: "p1-12", title: "Row Level Security (RLS) policies", description: "RLS for all tables: users see own data, admins see all, public read for published content, hosts manage own shows/mixes", status: "done", icon: Lock, tags: ["db", "security"], effort: "XL" },
      ...
      { id: "p1-14", title: "Generate shared TypeScript types", description: "Run Supabase type generation, export to packages/types. Create Zod validation schemas for all tables", status: "done", icon: Code2, tags: ["types"], effort: "M" },
    ],
  },
  ...
  // =========================================================================
  {
    ...
    icon: Server,
    color: "#7401df",
    timeline: "Week 3-5 — In Progress",
    tasks: [
      { id: "p2-1", title: "Auth module — registration, login, profile", description: "POST /auth/register, POST /auth/login, GET /users/me, PATCH /users/me, GET /users/me/roles", status: "done", icon: Shield, tags: ["api", "auth"], effort: "L", deps: ["p1-1", "p1-2", "p1-13"] },
      { id: "p2-2", title: "Streams module — CRUD + live status", description: "GET /streams, GET /streams/:id, POST /streams (admin), PATCH /streams/:id (admin). WebSocket endpoint for real-time listener counts", status: "done", icon: Radio, tags: ["api"], effort: "L", deps: ["p1-3"] },
      { id: "p2-3", title: "Shows module — CRUD + schedule", description: "GET /shows, GET /shows/:id, POST /shows (host), PATCH /shows/:id. GET /schedule, POST /schedule/slots (admin)", status: "done", icon: CalendarDays, tags: ["api"], effort: "L", deps: ["p1-4"] },
      { id: "p2-4", title: "Mixes module — upload + streaming", description: "GET /mixes, GET /mixes/:id, POST /mixes (host), file upload to Supabase Storage with presigned URLs, POST /mixes/:id/play for play count tracking", status: "done", icon: Music, tags: ["api", "storage"], effort: "XL", deps: ["p1-5"] },
      { id: "p2-5", title: "Podcasts module — series + episodes", description: "Full CRUD for podcast_series and episodes. File upload for audio. Auto-generate RSS feed endpoint GET /podcasts/:slug/feed.xml", status: "done", icon: Mic, tags: ["api", "storage"], effort: "XL", deps: ["p1-6"] },
      { id: "p2-6", title: "Events module — CRUD + registration", description: "GET /events, POST /events, PATCH /events/:id. POST /events/:id/register, GET /events/:id/ticket, POST /events/:id/checkin", status: "done", icon: Ticket, tags: ["api"], effort: "L", deps: ["p1-7"] },
      { id: "p2-7", title: "Points module — earn, spend, history", description: "GET /points/balance, GET /points/history, POST /points/earn (internal), GET /rewards, POST /rewards/:id/redeem", status: "done", icon: Star, tags: ["api"], effort: "L", deps: ["p1-8"] },
      { id: "p2-8", title: "Advertising module — campaigns + creatives", description: "Full CRUD for campaigns and creatives. POST /ads/impression for tracking. GET /ads/report/:campaign_id for performance reports", status: "done", icon: Megaphone, tags: ["api"], effort: "XL", deps: ["p1-9"] },
      { id: "p2-9", title: "Favorites & follows module", description: "POST /favorites, DELETE /favorites/:id, GET /favorites. POST /follows/:userId, DELETE /follows/:userId, GET /following", status: "done", icon: Heart, tags: ["api"], effort: "M", deps: ["p1-10"] },
      { id: "p2-10", title: "Directory module — listings + claims", description: "GET /directory, POST /directory/claim/:id, PATCH /directory/:id (owner), admin approval endpoints", status: "done", icon: MapPin, tags: ["api"], effort: "M", deps: ["p1-11"] },
      { id: "p2-11", title: "Admin module — user mgmt + moderation", description: "GET /admin/users, PATCH /admin/users/:id/roles, POST /admin/users/:id/suspend, GET /admin/content/pending, POST /admin/content/:id/approve", status: "done", icon: Shield, tags: ["api", "admin"], effort: "L", deps: ["p1-12"] },
  ...
  // =========================================================================
  {
    ...
    tasks: [
      ...
      { id: "p3-3", title: "User registration & login flow", description: "Email/password registration with email verification. Login with redirect. Password reset flow. Social auth (Google, Apple)", status: "done", icon: Users, tags: ["frontend", "auth"], effort: "L", deps: ["p2-1"] },
      { id: "p3-4", title: "User profile page & editor", description: "Public profile page with avatar, bio, listening stats, badges. Profile editor with image upload, social links", status: "done", icon: Users, tags: ["frontend"], effort: "M", deps: ["p2-1"] },
      { id: "p3-5", title: "Favorites system", description: "Heart button on shows, mixes, podcasts, DJs. Favorites page with filters by type. Quick-access in sidebar", status: "done", icon: Heart, tags: ["frontend"], effort: "M", deps: ["p2-9"] },
      { id: "p3-6", title: "mY1045 Points dashboard", description: "Points balance, tier status (Bronze/Silver/Gold/Platinum), transaction history, earning actions list, progress to next tier", status: "done", icon: Star, tags: ["frontend"], effort: "L", deps: ["p2-7"] },
      { id: "p3-7", title: "Rewards catalog & redemption", description: "Browse rewards with point costs, stock levels. Redeem with confirmation flow. Redemption history", status: "done", icon: Gift, tags: ["frontend"], effort: "M", deps: ["p2-7"] },
      { id: "p3-8", title: "Event discovery & registration", description: "Browse events with date/type/location filters. Event detail page with RSVP/ticket purchase. QR code ticket generation", status: "done", icon: Ticket, tags: ["frontend"], effort: "L", deps: ["p2-6"] },
      { id: "p3-9", title: "My Tickets page", description: "View upcoming/past event tickets, download QR codes, add to calendar, view event details", status: "done", icon: Ticket, tags: ["frontend"], effort: "S", deps: ["p2-6"] },
  ...
  // =========================================================================
  {
    ...
    tasks: [
      ...
      { id: "p4-2", title: "Show management CRUD", description: "Create/edit shows with title, description, genre, cover art. View show schedule and listener stats", status: "done", icon: Radio, tags: ["frontend"], effort: "L", deps: ["p2-3"] },
      { id: "p4-3", title: "Mix upload with processing", description: "Drag-and-drop upload (MP3/WAV/FLAC up to 500MB). Progress bar, waveform generation, auto-metadata extraction", status: "done", icon: Upload, tags: ["frontend", "storage"], effort: "XL", deps: ["p2-4"] },
      { id: "p4-4", title: "Mix editor & tracklist builder", description: "Edit mix metadata, cover art, tracklist. Searchable track database. Genre/BPM tagging", status: "done", icon: Music, tags: ["frontend"], effort: "L", deps: ["p2-4"] },
      ...
      { id: "p4-7", title: "Schedule view + calendar sync", description: "Weekly/monthly calendar of assigned show times. Export iCal feed for Google/Apple Calendar", status: "done", icon: CalendarDays, tags: ["frontend"], effort: "M", deps: ["p2-3"] },
  ...
  // =========================================================================
  {
    ...
    icon: Mic,
    color: "#7401df",
    timeline: "Week 6-8 — In Progress",
    tasks: [
      { id: "p5-1", title: "Podcast series CRUD", description: "Create/manage podcast series with cover art, description, category. Public series page with episode list", status: "done", icon: Mic, tags: ["frontend"], effort: "L", deps: ["p2-5"] },
      { id: "p5-2", title: "Episode publishing workflow", description: "Upload audio, add show notes with rich text editor, timestamps, guest credits. Schedule future publish dates", status: "done", icon: Upload, tags: ["frontend", "storage"], effort: "XL", deps: ["p2-5"] },
      ...
      { id: "p5-4", title: "Music track uploads", description: "Upload original tracks with metadata (title, artist, album, genre, BPM, key). Cover art upload", status: "done", icon: Music, tags: ["frontend", "storage"], effort: "L", deps: ["p2-4"] },
      ...
      { id: "p5-6", title: "Follower management", description: "View followers list, growth chart, most engaged fans. Notification preferences per follower", status: "done", icon: Users, tags: ["frontend"], effort: "M", deps: ["p2-9"] },
  ...
  // =========================================================================
  {
    ...
    icon: Megaphone,
    color: "#ef4444",
    timeline: "Week 7-10 — In Progress",
    tasks: [
      { id: "p6-1", title: "Advertiser self-serve campaign builder", description: "Step-by-step wizard: ad type (audio/banner/sponsorship), dates, budget, targeting (geo, age, daypart), frequency caps", status: "done", icon: Target, tags: ["frontend"], effort: "XL", deps: ["p2-8"] },
      { id: "p6-2", title: "Ad creative upload & approval", description: "Upload audio spots (15/30/60s) and display banners. Admin approval workflow with feedback loop", status: "done", icon: Image, tags: ["frontend", "storage"], effort: "L", deps: ["p2-8"] },
      { id: "p6-3", title: "Campaign dashboard (advertiser)", description: "View active/paused/completed campaigns. Key metrics: impressions, CTR, spend, pacing", status: "done", icon: BarChart3, tags: ["frontend"], effort: "L", deps: ["p2-8"] },
      ...
      { id: "p6-6", title: "Rate card management", description: "Admin-editable rate cards for 15s/30s/60s spots, sponsorships, digital banner ads. Tiered pricing", status: "done", icon: DollarSign, tags: ["frontend", "api"], effort: "M" },
      ...
      { id: "p6-8", title: "Invoicing & billing portal", description: "Auto-generated invoices, payment method management (Stripe), spending alerts, tax documents", status: "done", icon: CreditCard, tags: ["frontend", "api"], effort: "XL" },
  ...
  // =========================================================================
  {
    ...
    icon: Shield,
    color: "#10b981",
    timeline: "Week 8-10 — In Progress",
    tasks: [
      { id: "p7-1", title: "User management panel", description: "Search/filter users, view profiles, edit roles, suspend/ban accounts with reason tracking. Bulk actions", status: "done", icon: Users, tags: ["frontend", "api"], effort: "L", deps: ["p2-11"] },
      { id: "p7-2", title: "Content moderation queue", description: "Unified queue for pending mixes, podcasts, events, directory claims. Approve/reject with feedback", status: "done", icon: FileText, tags: ["frontend", "api"], effort: "L", deps: ["p2-11"] },
      { id: "p7-3", title: "Stream monitoring dashboard", description: "Real-time view of all 6 streams: listener counts, bitrate, uptime, error alerts. Auto-restart controls", status: "done", icon: Radio, tags: ["frontend", "realtime"], effort: "L", deps: ["p2-2"] },
      { id: "p7-4", title: "Broadcast schedule editor", description: "Drag-and-drop weekly schedule grid. Assign shows to time slots. Conflict detection. Bulk schedule templates", status: "done", icon: CalendarDays, tags: ["frontend"], effort: "L", deps: ["p2-3"] },
      ...
      { id: "p7-7", title: "Audit log viewer", description: "Searchable log of all admin actions with timestamps, user, action type, and affected resources", status: "done", icon: FileText, tags: ["frontend", "api"], effort: "M" },
      { id: "p7-8", title: "Ad approval workflow", description: "Review pending ad creatives. Preview in-context (banner placement, audio playback). Approve/reject with notes", status: "done", icon: Megaphone, tags: ["frontend"], effort: "M", deps: ["p2-8"] },
    ],
  },
  ...
  // =========================================================================
  {
    ...
    icon: Users,
    color: "#f97316",
    timeline: "Week 9-11 — In Progress",
    tasks: [
      { id: "p8-1", title: "Government services directory (backend)", description: "Connect existing 8-county frontend directory to Supabase. Claim workflow with admin approval", status: "done", icon: MapPin, tags: ["frontend", "api"], effort: "L", deps: ["p2-10"] },
      ...
      { id: "p8-6", title: "Push notifications system", description: "Browser push notifications for live shows, new content, event reminders, points earned. Preference management", status: "done", icon: Bell, tags: ["frontend", "api"], effort: "L" },
