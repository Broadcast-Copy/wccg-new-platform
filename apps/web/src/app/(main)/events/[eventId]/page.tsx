 ...
 * event so `/events/<id>` builds to static HTML — which makes the link a normal
 * client navigation AND makes a hard load / refresh resolve correctly (the
 * `_placeholder`-only approach made useParams() return "_placeholder", so the
 * page showed "Event not found"). The `_placeholder` entry is always kept so the
 * .htaccess SPA fallback still covers any event published after the last build —
...
export async function generateStaticParams(): Promise<{ eventId: string }[]> {
  const params: { eventId: string }[] = [{ eventId: "_placeholder" }];
  ...
  try {
    ...
    const res = await fetch(
      `${url}/rest/v1/events?select=id&status=eq.PUBLISHED&visibility=eq.PUBLIC`,
      {
    ...
    // any real event still resolves client-side via the SPA fallback.
  }
