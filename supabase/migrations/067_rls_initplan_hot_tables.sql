...
DROP POLICY IF EXISTS "Users can read own points history" ON public.points_history;
...
DROP POLICY IF EXISTS "Service role manages points history" ON public.points_history;
...
DROP POLICY IF EXISTS "Service role manages points" ON public.user_points;
CREATE POLICY "Service role manages points" ON public.user_points
  FOR ALL USING ((SELECT auth.role()) = 'service_role');

-- ===== listening_sessions =====
DROP POLICY IF EXISTS "Users can read own listening sessions" ON public.listening_sessions;
...
-- ===== dj_drops (188 rows) =====
DROP POLICY IF EXISTS "DJs read own drops" ON public.dj_drops;
...
DROP POLICY IF EXISTS "DJs insert own drops" ON public.dj_drops;
CREATE POLICY "DJs insert own drops" ON public.dj_drops
  FOR INSERT TO authenticated WITH CHECK (
    EXISTS (SELECT 1 FROM public.djs
            WHERE djs.id = dj_drops.dj_id AND djs.user_id = (SELECT auth.uid()))
  );
DROP POLICY IF EXISTS "DJs update own drops" ON public.dj_drops;
CREATE POLICY "DJs update own drops" ON public.dj_drops
  FOR UPDATE TO authenticated USING (
    EXISTS (SELECT 1 FROM public.djs
            WHERE djs.id = dj_drops.dj_id AND djs.user_id = (SELECT auth.uid()))
  );
DROP POLICY IF EXISTS "Admins read all drops" ON public.dj_drops;
...
DROP POLICY IF EXISTS "Admins update all drops" ON public.dj_drops;
...
DROP POLICY IF EXISTS "Production reads all drops" ON public.dj_drops;
...
-- ===== content_plays =====
DROP POLICY IF EXISTS "Users view own plays" ON public.content_plays;
...
DROP POLICY IF EXISTS "Owners can update their listings" ON public.directory_listings;
CREATE POLICY "Owners can update their listings" ON public.directory_listings
  FOR UPDATE TO authenticated USING (owner_id = (SELECT auth.uid()));
DROP POLICY IF EXISTS "Owners can delete their listings" ON public.directory_listings;
...
CREATE POLICY "djs_service_write" ON public.djs
  FOR ALL USING ((SELECT auth.role()) = 'service_role');
DROP POLICY IF EXISTS "Staff manage djs" ON public.djs;
...
CREATE POLICY "dj_slots_service_write" ON public.dj_slots
  FOR ALL USING ((SELECT auth.role()) = 'service_role');
DROP POLICY IF EXISTS "Production manages slots" ON public.dj_slots;
...
DROP POLICY IF EXISTS "Admins can insert navigation" ON public.site_navigation;
...
DROP POLICY IF EXISTS "Admins can update navigation" ON public.site_navigation;
...
DROP POLICY IF EXISTS "Admins can delete navigation" ON public.site_navigation;
...
DROP POLICY IF EXISTS "Admins can manage invite codes" ON public.employee_invite_codes;
