-- Live migration export: version 20260328155250, name "initial_schema"
...
CREATE TYPE favorite_target_type AS ENUM ('STREAM', 'SHOW');
CREATE TYPE points_reason AS ENUM ('LISTENING', 'EVENT_CHECKIN', 'PURCHASE', 'REDEMPTION', 'ADMIN_GRANT', 'SIGNUP');
CREATE TYPE points_trigger_type AS ENUM ('LISTEN_MINUTES', 'EVENT_ATTENDANCE', 'PURCHASE', 'SIGNUP');
...
CREATE TYPE organizer_role AS ENUM ('OWNER', 'COHOST', 'STAFF');
...
CREATE TABLE streams (
  ...
  sort_order INTEGER NOT NULL DEFAULT 0, image_url TEXT,
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(), updated_at TIMESTAMPTZ NOT NULL DEFAULT now()
);
CREATE TABLE stream_sources (
  id TEXT PRIMARY KEY DEFAULT gen_random_uuid()::text,
  stream_id TEXT NOT NULL UNIQUE REFERENCES streams(id) ON DELETE CASCADE,
  primary_url TEXT, fallback_url TEXT, mount_point TEXT,
  format TEXT DEFAULT 'audio/mpeg', bitrate INTEGER DEFAULT 128,
...
CREATE TABLE stream_metadata (
  id TEXT PRIMARY KEY DEFAULT gen_random_uuid()::text,
  stream_id TEXT NOT NULL UNIQUE REFERENCES streams(id) ON DELETE CASCADE,
  current_title TEXT, current_artist TEXT, current_track TEXT,
  album_art TEXT, listener_count INTEGER DEFAULT 0,
  is_live BOOLEAN NOT NULL DEFAULT false, last_updated TIMESTAMPTZ DEFAULT now()
);
...
CREATE TABLE hosts (
  id TEXT PRIMARY KEY DEFAULT gen_random_uuid()::text,
  name TEXT NOT NULL, slug TEXT NOT NULL UNIQUE, bio TEXT,
  avatar_url TEXT, email TEXT, is_active BOOLEAN NOT NULL DEFAULT true,
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(), updated_at TIMESTAMPTZ NOT NULL DEFAULT now()
);
...
CREATE TABLE show_episodes (
  id TEXT PRIMARY KEY DEFAULT gen_random_uuid()::text,
  show_id TEXT NOT NULL REFERENCES shows(id) ON DELETE CASCADE,
  title TEXT NOT NULL, description TEXT, air_date TIMESTAMPTZ,
  duration INTEGER, audio_url TEXT, image_url TEXT,
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(), updated_at TIMESTAMPTZ NOT NULL DEFAULT now()
);
...
CREATE TABLE schedule_blocks (
  ...
  is_override BOOLEAN NOT NULL DEFAULT false, override_date DATE,
  is_active BOOLEAN NOT NULL DEFAULT true, color TEXT DEFAULT '#3B82F6',
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(), updated_at TIMESTAMPTZ NOT NULL DEFAULT now()
);
...
CREATE TABLE listening_history (
  ...
  duration INTEGER DEFAULT 0, created_at TIMESTAMPTZ NOT NULL DEFAULT now()
);
...
CREATE TABLE points_ledger (
  id TEXT PRIMARY KEY DEFAULT gen_random_uuid()::text,
  user_id UUID NOT NULL REFERENCES profiles(id) ON DELETE CASCADE,
  amount INTEGER NOT NULL, reason points_reason NOT NULL,
...
CREATE TABLE points_rules (
  id TEXT PRIMARY KEY DEFAULT gen_random_uuid()::text,
  name TEXT NOT NULL, trigger_type points_trigger_type NOT NULL,
  points_amount INTEGER NOT NULL, threshold INTEGER DEFAULT 0,
  is_active BOOLEAN NOT NULL DEFAULT true, cooldown_minutes INTEGER DEFAULT 0,
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(), updated_at TIMESTAMPTZ NOT NULL DEFAULT now()
);
CREATE TABLE reward_catalog (
  id TEXT PRIMARY KEY DEFAULT gen_random_uuid()::text,
  name TEXT NOT NULL, description TEXT, image_url TEXT,
  points_cost INTEGER NOT NULL, category TEXT,
  stock_count INTEGER DEFAULT -1, is_active BOOLEAN NOT NULL DEFAULT true,
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(), updated_at TIMESTAMPTZ NOT NULL DEFAULT now()
);
...
CREATE TABLE events (
  ...
  image_url TEXT, banner_url TEXT, venue TEXT, address TEXT,
  city TEXT, state TEXT, zip_code TEXT,
  latitude DOUBLE PRECISION, longitude DOUBLE PRECISION,
  start_date TIMESTAMPTZ NOT NULL, end_date TIMESTAMPTZ NOT NULL,
  timezone TEXT NOT NULL DEFAULT 'America/New_York',
  category TEXT, status event_status NOT NULL DEFAULT 'DRAFT',
  visibility event_visibility NOT NULL DEFAULT 'PUBLIC',
  max_attendees INTEGER, is_free BOOLEAN NOT NULL DEFAULT true,
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(), updated_at TIMESTAMPTZ NOT NULL DEFAULT now()
);
...
CREATE TABLE ticket_types (
  ...
  name TEXT NOT NULL, price DECIMAL(10,2) NOT NULL DEFAULT 0,
  quantity INTEGER NOT NULL DEFAULT 0, quantity_sold INTEGER NOT NULL DEFAULT 0,
  description TEXT, sales_start TIMESTAMPTZ, sales_end TIMESTAMPTZ,
  is_active BOOLEAN NOT NULL DEFAULT true,
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(), updated_at TIMESTAMPTZ NOT NULL DEFAULT now()
);
CREATE TABLE event_registrations (
  ...
  qr_code TEXT, purchased_at TIMESTAMPTZ NOT NULL DEFAULT now(),
