-- Live migration export: version 20260328160140, name "seed_data"
...
INSERT INTO role_permissions (role_id, permission_id) SELECT 'role_editor', id FROM permissions WHERE name IN ('streams.update', 'shows.create', 'shows.update', 'hosts.manage', 'schedule.manage');

INSERT INTO streams (id, name, slug, description, category, status, sort_order) VALUES
  ('stream_main', 'WCCG 104.5 FM', 'wccg-main', 'Fayetteville Hip Hop Station', 'MAIN', 'ACTIVE', 1),
  ('stream_gospel', 'Gospel Classics', 'gospel-classics', 'Timeless gospel hymns and classics', 'GOSPEL', 'ACTIVE', 2),
  ('stream_hiphop', 'Holy Hip Hop', 'holy-hip-hop', 'Christian hip hop and rap', 'HIP_HOP', 'ACTIVE', 3),
  ('stream_rnb', 'Inspirational R&B', 'inspirational-rnb', 'Uplifting R&B and soul', 'RNB', 'ACTIVE', 4),
  ('stream_talk', 'WCCG Talk', 'wccg-talk', 'Community discussions and talk shows', 'TALK', 'ACTIVE', 5);
...
INSERT INTO shows (id, name, slug, description, is_active) VALUES
  ('show_morning', 'Gospel Morning', 'gospel-morning', 'Start your day with uplifting gospel music', true),
  ('show_midday', 'Midday Praise', 'midday-praise', 'Your midday praise break', true),
  ('show_drive', 'Drive Time Gospel', 'drive-time-gospel', 'Gospel music for your commute', true),
  ('show_night', 'Quiet Storm Gospel', 'quiet-storm-gospel', 'Smooth gospel for the evening', true),
  ('show_sunday', 'Sunday Morning Worship', 'sunday-worship', 'Live worship every Sunday', true),
  ('show_talk', 'Community Voice', 'community-voice', 'Community discussions', true),
  ('show_youth', 'Next Gen Praise', 'next-gen-praise', 'Gospel for the younger generation', true),
...
INSERT INTO hosts (id, name, slug, bio, is_active) VALUES
  ('host_james', 'Brother James', 'brother-james', 'Morning show host for 15+ years', true),
  ('host_sarah', 'Sister Sarah', 'sister-sarah', 'Midday praise host and community advocate', true),
  ('host_mike', 'DJ Mike G', 'dj-mike-g', 'Drive time host with 20 years in gospel radio', true),
  ('host_lisa', 'Lisa Grace', 'lisa-grace', 'Evening host specializing in smooth gospel', true),
  ('host_williams', 'Pastor D. Williams', 'pastor-williams', 'Community voice host', true),
  ('host_tasha', 'Tasha Joy', 'tasha-joy', 'Youth programming host', true);
...
DO $$ DECLARE d INTEGER;
BEGIN FOR d IN 1..5 LOOP
